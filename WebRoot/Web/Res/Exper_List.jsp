...
<head>
	...
	<script language="javascript">
		...
		//提交查询数据列表
		function querydata(){
			document.form2.action="<c:url value='/pt/con/listcontentexper.shtml'/>";
    ...
    <div class="main">
        ...
        <div class="content">
        	<div class="leftsidebar">
                <span><img src="<c:url value='/Web/Image/menu-dxjyk.png'/>" /></span>
                <ul class="ulsidebar">
                	<c:choose>
	                	...
	                	<c:forEach items="${codeList}" var="d">                	
	                    	<li><a href="<c:url value='/pt/con/listcontentexper.shtml?typeid=${d.id }'/>"><label class="${d.id==typeid?'selected':'' }"></label>${d.codename }</a></li>
	                	</c:forEach>                		
                		</c:otherwise>
                	</c:choose>
                </ul>
                        ...
                        <c:forEach items="${dataList}" var="d" varStatus="order">
                       	<tr>                    		
                             <td align="left" style="padding-left:10px;">
                             	<a href="<c:url value='/pt/con/experview.shtml'/>?pkId=${d.id }&fatherid=${fatherid }&typeid=${typeid }&KIND=${KIND }" class="title" target="_blank" title="${d.title }">
	                             	<c:choose>
