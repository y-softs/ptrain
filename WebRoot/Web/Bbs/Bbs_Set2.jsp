...
<head>
    ...
    <script type="text/javascript">
		...
		function validateInfo(){			
			...
			$("#title").attr("dataType","Require").attr("msg","资源名称<fmt:message key="format.null" bundle="${bms}"/>");//验证附件
			$("#content").attr("dataType","Require").attr("msg","资源简介<fmt:message key="format.null" bundle="${bms}"/>");//验证附件
			var jsonArr = [{"key":"upfile","error":"授课内容文件路径不能为空！"}];
			for(i =0;i<jsonArr.length;i++){
				if($("#t_"+jsonArr[i]['key']).size()==0 && ($("#"+jsonArr[i]['key']+"FileList").html()==null || $("#"+jsonArr[i]['key']+"FileList").html()=="")){
					parent.layer.msg('<div style="color:red;font-size:13px;">'+jsonArr[i]['error']+'</div>' ,2,3); 
					return false;
				}
			}
        ...
        <tr>
        	<td>
                ...
                <a href="###" class="btn btn-inverse" onclick="layerClose();">关闭</a>
                 ...
                 <input type="hidden" name="ptrainBbsBean.downnum" value="0" />
            </td>
        </tr>
    </table>
</form>
