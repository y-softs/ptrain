...
<div class="container">
	...
	<script type="text/javascript">
		...
		function setDataWin(id){
			var url = "<c:url value='/Ptrain/Req/setptrainreqchk.shtml'/>?ptrainReqBean.id="+id;
			url+="&querymap.unitid=${querymap.unitid}";
			url+="&setpage=/Ptrain/Request/Conte_Uchk_Win3.jsp";
        	parent.$.jBox.open("iframe:"+url, ("菜单审批-修改"), 650, 450, { buttons: {}});
		}
		
		
	    //列
	    var cols = [
	        { title:'专业类别', name:'specid',width:80, align:'center',renderer: null},
	        { title:'项目名称', name:'itemname' ,width:250, align:'left', renderer: function(val,item,rowIndex){
	        	return "<a href=\"JavaScript:setDataWin('"+item.id+"');\">"+val+"</a>";
	        }},
	        { title:'发起人', name:'requserid' ,width:50, align:'center', renderer: function(val,item,rowIndex){
	        	return val||"&nbsp;";
	        }}
	    ];
	    //分页
	    var paramsV={
	    	...
	    	'ptrainReqBean.id':'${ptrainReqBean.id}'
	    }
	    var mmg = $('#dataTable').mmGrid({
	        ...
	        width:'98%',
	        cols: cols,
	        url: '<c:url value="/Ptrain/Req/listptrainreqbyids.shtml"/>',
	        params:paramsV,
	        method: 'post',
			nowarp:true
	    });
