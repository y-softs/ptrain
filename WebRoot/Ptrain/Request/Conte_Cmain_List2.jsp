...
<div id="container">
    ...
    <!-- 标题区域 --->
    <div class="titleArea">
        <div class="titleClass">培训中心加菜[<span class="switchSearch">数据搜索</span>]</div>
    </div>
    <!-- 分隔符 --->
    <div class="lineSpac"></div>
    <!-- 查询条件 --->
...
<script language="javascript">
    ...
    var cols = [
        ...
        { title:'课程介绍', name:'itemdesc' ,width:220, align:'left', renderer: function(val,item,rowIndex){
        	return (val||"&nbsp;")+'<font style="display:none">'+item.id+'</font>';
        }},
        { title:'培训天数', name:'daycount' ,width:60, align:'center', renderer: function(val,item,rowIndex){
        	var r=('-1'==item.datasign&&''==val)?'<span class="fontcolor_red_b">！</span>':''
        	return r+val;
        }},
        { title:'操作', name:'' ,width:130, align:'center', renderer: function(val,item,rowIndex){
    ...
    function setData(id,fun){
    	var paramV="querymap.unitid=${querymap.unitid}&ptrainReqtempBean.id="+id+"&querymap.reqtype=${querymap.reqtype}&querymap.specid="+$("#specid").val()+"&setpage=/Ptrain/Request/Conte_Cmain_Set1.jsp&fun="+fun;
        parent.$.jBox.open("iframe:<c:url value="/Ptrain/Req/setptrainreqtempwin.shtml"/>?"+paramV, 
        (fun == 1?"新增":"培训中心加菜-修改"), 900, 450, { buttons: {}});
    }
    //删除
    function delData(id){
    	id = id || "-1";
