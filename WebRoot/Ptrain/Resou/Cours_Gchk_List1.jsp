...
<div id="container">
    ...
    <div class="mySearchMain" id="mySearchMain">
        <form method="post" name="form" id="form1" action="">
            ...
            <input type="hidden" name="listpage" value="/Ptrain/Resou/Cours_Gchk_List1.jsp" id="listpage" />
...
<script language="javascript">
    ...
    var cols = [
        ...
        { title:'操作', name:'' ,width:240, align:'left', renderer: function(val,item,rowIndex){
        	var r='';
	...
	function chkData(id,sign){
	    var newid = id,setpage='/Ptrain/Resou/Cours_Gchk_Win1.jsp',querylist='';
		if('1'==sign){//审核
	        if(''==id){
				...
				newid = idAttr.join(",");
				setpage='/Ptrain/Resou/Cours_Gchk_Win2.jsp';
				querylist='true';
	        }
			var url = "<c:url value='/Ptrain/Cours/setptraincourschk.shtml'/>?ptrainCoursBean.id="+newid;
			...
			parent.$.jBox.confirm("确定退回记录？", "提示", function (v, h, f) {
	            	if (v == 'ok'){
						$("#chksign").val('${TUI_HUI}');
					    $("#id").val(newid);
						document.form1.action="<c:url value='/Ptrain/Cours/updateptraincourschk.shtml'/>";
    ...
    function dataDeta(id){
		var url="<c:url value='/Ptrain/Cours/setptraincours.shtml'/>?setpage=/Ptrain/Resou/Cours_Esta_Win.jsp&ptrainCoursBean.id="+id;
