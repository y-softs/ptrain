    ...
    MMGrid.prototype = {
        ...
        , _initOptions: function(){
            ...
            if($("#mySearchMain").size()>0){
               ...
               //$backboard.append($("#mySearchMain").html());
               //$("#mySearchMain").html('');
            }
        ...
        , _initEvents: function(){
            ...
            //绑定搜索切换功能
            $(".switchSearch").bind("click",function(){
                if($(this).html() == "数据搜索"){
                    $(this).html("关闭搜索");
                    downGrid();
                }else{
            ...
            //向下按钮
            var $btnBackboardDn = $mmGrid.find('a.mmg-btnBackboardDn').on('click', downGrid);
            function downGrid(){
            ...
            //向上按钮
            $mmGrid.find('a.mmg-btnBackboardUp').on('click',upGrid);
            function upGrid(){
        ...
        , _loadAjax: function(args){
            ...
            if(opts.remoteSort){
                ...
                if(sortName){
                    ...
                    params.sortfield = sortName;
                    if(sortName.lastIndexOf('desc')<0&&sortName.lastIndexOf('asc')<0){
                    	params.sortfield = sortName+' '+sortStatus;
                    }
                }
            }
            ...
            //查询参数合并
			var searchParams = {};
			//循环获取查询条件(mySearchMain)中的表单元素
			var formobj = $("#mySearchMain #form1")[0];
			//当页面中存在form表单对象时
			if("undefined" != typeof formobj || formobj == "undefined"){
				var elelen = formobj.length;
				var checkStr = ',';         //设置有获取checkbox值的临时字符串
				for(var i=0;i<elelen;i++){
					var obj = $(formobj[i]);   //循环表单中各个元素的jquery对象
					//如果当前表单元素为disabled时则继续下一次迭代,跳转本次循环操作
					if(obj.is(":disabled")){
						continue;
					}
					//当前表单元素类型为checkbox时
					if(formobj[i].type == 'checkbox'){
						if($(formobj[i]).is(":checked")){
							var chkname = obj.attr("name");    //获取选中checkbox的表单名称元素
							if(checkStr.indexOf(','+chkname+',')>=0){    //判断当前checkbox表单元素名称是否存在相同的[如果有则在后面追加]
								var tempval = searchParams[obj.attr("name")];
								searchParams[obj.attr("name")] = tempval+','+obj.val();
							}else{
								searchParams[obj.attr("name")]=obj.val();
								checkStr += obj.attr("name")+',';  //将有设置checkbox的值的表单元素名称name添加到临时字符串中[用来判断验证表单元素中相同名称的checkbox]
							}
						}
					//当前表单元素类型为radio时
						...
						if($('input:radio[name="'+obj.attr("name")+'"]').is(":checked")){
							var radioval = $('input:radio[name="'+obj.attr("name")+'"]:checked').val();
							searchParams[obj.attr("name")]=radioval;
						}
					//当前表单元素类型为input,select,hidden,button
					}else{
						searchParams[obj.attr("name")]=obj.val();
					}
				}
				//合并查询条件中search参数[注意:如果参数中的key相同时会进行覆盖]
			...
			//合并搜索的参数
            if("undefined" != typeof _searparam){
				//params = $.extend(params, _searparam);
			}
